"""
 This file plugs into the existing backend at exactly two call sites:
...
# Just like in the columns in EM_data + AQ_data + OC_data.
...
class ForecastService:
    """
    Wraps the "backend" stack for use inside the EquiTwin FastAPI backend.
...
def matches_row_to_ingest(row: Dict[str, Any]) -> Dict[str, Any]:
    """
    Map a raw row from the table (JOIN of EM + AQ + OC data)
