"""
equitwin_integration/train_all.py
==================================
...
Usage (Python)
--------------
    from equitwin_integration.train_all import train_all_features

    # Quick run (default — recommended for first try)
    results = train_all_features(
        db_url="postgresql+psycopg2://user:pass@localhost:5432/eco_init",
        table="matches",
    )
...
# Why these numbers?
#
#  limit=600  →  ~600 rows × ~9 models × 3 folds × 3 ST horizons × 2 levels
#              ≈ 324 fits, each on ≤600 rows → finishes in 1-3 minutes.
#
#  Fast models: ridge (instant), hgb (fast tree), rf (parallel trees).
#  Excluded from fast: gp (O(n³) — brutal on CPU), ann (slow convergence),
#                      voting (trains 3 models per fold), lasso/elastic (slower
#                      than ridge with no benefit at this scale).
#
#  Reduced horizons: ST [1,3,6] covers immediate / 45-min / 90-min lookahead —
#                    enough for InnerMPC. LT [1,3] covers 4h / 12h for OuterMPC.
# ---------------------------------------------------------------------------
...
def train_all_features(
    ...
    *,
    ...
    # Print a clear summary so the user knows what they're getting into
...
def main() -> None:
    ...
    # Start from the chosen preset, then apply any explicit overrides
