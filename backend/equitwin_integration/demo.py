"""
equitwin_integration/demo.py
=============================
End-to-end walkthrough of the EquiTwin integration.
...
def build_stack(artifacts_root: str = "artifacts"):
    ...
    signal_cols = [
        ...
        # Weather exogenous — injected per tick via WeatherClient (NaN when unavailable)
        "outdoor_temp",
        "weather_condition",
        "sunlight",
    ]
    ...
    # so we filter to the available subset first (same pattern as bootstrap.py).
...
def main():
    ...
    try:
        ...
        # Fetch weather forecast if client is available (falls back to None gracefully)
