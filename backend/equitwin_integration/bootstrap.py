"""
equitwin_integration/bootstrap.py
==================================
...
Typical EquiTwin startup
------------------------
    from equitwin_integration.bootstrap import build_equitwin_stack, EquiTwinConfig
    from equitwin_mpc.hierarchical import OuterMPC, InnerMPC

    cfg = EquiTwinConfig(artifacts_root="artifacts")
    stack = build_equitwin_stack(cfg)

    outer_mpc = OuterMPC(lt_steps=stack.hz.lt_horizons)
    inner_mpc = InnerMPC(st_steps=stack.hz.st_horizons)

Control tick (called at every 15-minute data arrival)
-----------------------------------------------------
    stack.ingest(sensor_row)                          # feed latest sensor reading

    bundle = stack.coordinator.forecast_now(group_id) # get forecasts
    plan   = outer_mpc.solve(bundle, state)           # 4h references
    action = inner_mpc.solve(bundle, state, plan)     # 15m control output

Notes
-----
* `group_id` is the `sensor_id` value in your sensor rows (default: "1").
* The stack is intentionally stateless between ticks – just keep calling ingest().
* Pass `features=["energy"]` to load only a subset; useful during development.
"""
...
def build_equitwin_stack(cfg: EquiTwinConfig) -> EquiTwinStack:
    """
    Build and return a ready-to-use EquiTwinStack.
    ...
    # We extend st_lags with a sentinel value so the ring-buffer is large enough.
