"""
equitwin_integration/tick_runner.py
=====================================
...
Minimal EquiTwin control loop example
--------------------------------------
    ...
    stack = build_equitwin_stack(EquiTwinConfig(artifacts_root="artifacts"))
    runner = TickRunner(stack)

    # At every 15-minute tick:
    output = runner.tick(
        sensor_row={
        ...
        state={"temp_target": 21.5},
    )
    print(output.inner_action.u)      # HVAC setpoints
    print(output.outer_plan.refs)     # 4h reference trajectories
"""
...
class TickRunner:
    ...
    def tick(
        ...
        -------
        ControlOutput
            Always returns an object.  Check ``warmed_up`` and ``error`` fields.
        """
