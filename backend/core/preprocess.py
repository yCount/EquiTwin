...
core/preprocess.py
==================
Pre-processing layer for the raw EquiTwin sensor table.

The raw 'matches' table is a wide, denormalised event log that stores multiple
sensor types in a single table.  Each row type fills only certain columns,
leaving the rest NULL.  Example sensor types:
...
Root-cause of "0 rows after featurisation" for occupancy
---------------------------------------------------------
  1. ``num_targets`` is stored as VARCHAR/TEXT in the DB  (e.g. the string
     "18") OR is always NULL because the occupancy sensor writes the count
     into a different column than the code expects.
  2. Cross-sensor columns (co2, temp, total_act_power) are NULL for occupancy
     rows because they come from different sensor types.

This module fixes both issues and applies general cleaning to every feature.
...
def preprocess_raw_table(
    ...
    *,
    ...
    Notes
    -----
    The function is idempotent: running it twice on already-clean data does
