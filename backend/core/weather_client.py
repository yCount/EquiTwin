...
Provides current conditions, hourly forecast, and historical data
for training enrichment.  Uses only Python stdlib — no new dependencies.

Configuration
-------------
Location is read from environment variables:
    WEATHER_LAT   latitude  (float, e.g. "52.37")
    WEATHER_LON   longitude (float, e.g. "4.89")

Usage
-----
    from core.weather_client import WeatherClient

    client = WeatherClient(lat=52.37, lon=4.89)
    snap = client.get_current()     # WeatherSnapshot (15-min cache)
    fc   = client.get_forecast(24)  # List[WeatherSnapshot]
    df   = client.get_historical_df("2024-01-01", "2024-12-31")
"""
...
def _wmo_to_condition(code: int) -> str:
    """Map WMO weather interpretation code to a fixed categorical string.
...
class WeatherClient:
    ...
    --------------------
    Every public method catches all exceptions and returns NaN-valued objects
    instead of raising, so the MPC loop is never blocked by a weather API outage.
    """
    ...
    def get_current(self) -> WeatherSnapshot:
        ...
        try:
            ...
            # shortwave_radiation is hourly — match by hour prefix
