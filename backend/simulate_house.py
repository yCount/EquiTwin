...
Occupancy: Scripted daily pattern
  00-07  2 people sleeping
  07-09  2 people (morning routine)
  09-17  0 people (away)
  17-22  2 people (evening)
  22-24  2 people (sleeping)

Usage
-----
    # Fast simulation, no weather (NaN)
    python simulate_house.py

    # With live weather from Open-Meteo
    set WEATHER_LAT=41.01 && set WEATHER_LON=28.98 && python simulate_house.py

    # Options
    python simulate_house.py --ticks 96         # 24h (default)
    python simulate_house.py --ticks 32         # 8h
    python simulate_house.py --speed 0.5        # 0.5 s delay per tick
    python simulate_house.py --setpoint 22.0    # comfort temp setpoint
    python simulate_house.py --group 1          # sensor group id
"""
...
# Fixed-width (4-char) display labels for each mode
...
class ScheduleConfig:
    """
    Commercial-building daily schedule.
    ...
    ─────────────────────────────
      ...
      POST: band widens linearly 3 → 6 °C — drifts toward night setpoint
...
def main() -> None:
    ...
    for tick in range(args.ticks):
        ...
        # 6. Build display mode string: 4-char phase label + "+" if MPC active
